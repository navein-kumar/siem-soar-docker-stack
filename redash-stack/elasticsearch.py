...
class BaseElasticSearch(BaseQueryRunner):
    ...
    def _get_query_mappings(self, url):
        ...
        for index_name in mappings_data:
            ...
            m_data = index_mappings.get("mappings", {})
            # ES 7+ / OpenSearch: mappings is a direct object with "properties"
            if "properties" in m_data:
                for property_name in m_data["properties"]:
                    property_data = m_data["properties"][property_name]
                    if isinstance(property_data, dict) and property_name not in mappings:
                ...
                # ES 5.x style: mappings contains doc types
                for m in m_data:
                    if not isinstance(m_data[m], dict) or "properties" not in m_data[m]:
                        continue
                    for property_name in m_data[m]["properties"]:
                        property_data = m_data[m]["properties"][property_name]
    ...
    def test_connection(self):
        try:
            r = requests.get("{0}/_cluster/health".format(self.server_url), auth=self.auth, verify=False)
            r.raise_for_status()
        except requests.HTTPError as e:
            logger.exception(e)
...
class ElasticSearch(BaseElasticSearch):
    ...
    def run_query(self, query, user):
        try:
            ...
            r = requests.get(url, json=query_dict, auth=self.auth, verify=False)
            r.raise_for_status()
